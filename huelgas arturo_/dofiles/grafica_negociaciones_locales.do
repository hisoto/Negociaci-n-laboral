...
gen fecha_lab=tm(`year_local'm`mes_local')

keep if fecha>=tm(`year_firstp'm`mes_firstp')

*Original
twoway scatter lmil fecha, ms(none) mlabc(black) mlabposition(2) ///
mlabsize(3.2) mlabel(lmil) mlabangle(vertical) legend(off) graphregion(color(white)) || ///
bar lmil fecha, yaxis(1) xlabel(,format(%tmCCYY)) color("152 152 154") ytitle("Trabajadores involucrados (miles)") ///
ylabel(0(50)220, nogrid angle(horizontal) labsize(5) axis(1)) || ///
scatter REAL fecha if fecha==fecha_lab , yaxis(2)  ms(none) mlabc("35 91 78") mlabposition(6) mlabsize(#9) mlabel(breal)  || ///
line REAL fecha, yaxis(2) yscale(alt) yscale(alt axis(1)) xlabel(, nogrid format(%tmCCYY)) ///
ylabel(, grid angle(horizontal) labsize(3.5) axis(2)) ///
color("35 91 78") ytitle("Incremento salarial real (%)", axis(2)) ///
xtitle("") ||, ysize(2.8) xsize(5.9) ylabel(, angle(horizontal) labsize(3.5) axis(1)) 
graph export "$graphs\negociaciones_locales/neg_local_`year_local'm`mes_local'.png", replace
graph export "$presentaciones/neg_local_`year_local'm`mes_local'.png", replace
